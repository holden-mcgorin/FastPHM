...
class Plotter:
    ...
    def feature(bearing: Bearing):
        ...
        if bearing.stage_data is not None:
        ...
        # 当轴承包含预测数据时将绘画轴承的预期曲线
        if bearing.predict_history is not None:
            # 画置信区间（不确定性预测）
            if bearing.predict_history.lower is not None and bearing.predict_history.upper is not None:
                plt.fill_between(
                    np.arange(len(bearing.predict_history.lower) + 1) + bearing.predict_history.begin_index - 1,
                    [float(bearing.feature_data.iloc[
                               bearing.predict_history.begin_index, 0])] + bearing.predict_history.lower,
                    [float(bearing.feature_data.iloc[
                               bearing.predict_history.begin_index, 0])] + bearing.predict_history.upper,
                    alpha=0.25,
                    label='confidence_interval')
            # 画预测值（确定性预测和不确定性预测）
            if bearing.predict_history.prediction is not None:
                plt.plot(
                    np.arange(len(bearing.predict_history.prediction) + 1) + bearing.predict_history.begin_index - 1,
                    [float(bearing.feature_data.iloc[bearing.predict_history.begin_index, 0])] +
                    bearing.predict_history.prediction,
                    label='prediction')

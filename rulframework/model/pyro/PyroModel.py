...
class PyroModel(ABCModel):
    ...
    def train(self, train_set: Dataset, epochs=100, batch_size=128, weight_decay=0, criterion=None, optimizer=None):
        ...
        :param batch_size:
        :param train_set:
        :param epochs:在这里为从马尔科夫链中采样的个数
        :return:
        """
        # 使用 HMC 进行 MCMC 推断
        hmc_kernel = HMC(self.pyro_model)
        mcmc = MCMC(hmc_kernel, num_samples=epochs, warmup_steps=200)
        x = torch.tensor(train_set.x, dtype=torch.float64, device=self.device)
        y = torch.tensor(train_set.y, dtype=torch.float64, device=self.device)
        mcmc.run(x, y)

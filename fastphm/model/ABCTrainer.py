...
class ABCTrainer(ABC):
    ...
    def train(self,
              model,
              train_set: Dataset) -> Union[List, Dict]:
        """
        训练接口
        :param model:
        :param train_set:
    ...
    def recursive_update(self, default: dict, custom: dict):
        """
        合并默认配置与已有配置
        :param default:
        :param custom:
        ...
        for key, value in default.items():
            if isinstance(value, dict):
                # 如果custom中没有这个键，直接赋值
                if key not in custom or not isinstance(custom[key], dict):
                    custom[key] = value
                else:
                    # 递归合并子字典
                    self.recursive_update(value, custom[key])
            else:
                # 不存在该项才补充
